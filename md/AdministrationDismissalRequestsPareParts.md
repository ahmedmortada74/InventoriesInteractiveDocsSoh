...
# AdministrationDismissalRequestsPareParts.aspx

## Overview

**File**: `\Inventories\Process\AdministrationDismissalRequestsPareParts.aspx`
**Purpose**: Spare parts dismissal request system for inventory items with equipment-based workflow
**Technology**: ASP.NET Web Forms with DevExpress Bootstrap controls (v18.2)
**Primary Users**: Inventory administrators, equipment managers, maintenance personnel

## User Input Requirements and Error Prevention

### Critical User Input Requirements

#### 1. **Equipment Selection (Required for Request)**
- **Equipment Dropdown**: Must select valid equipment for request
- **Error Prevention**: System validates equipment is selected before loading items
- **Data Source**: Inventories_Item_Settings table with equipment items
- **Default Behavior**: User must select equipment manually
- **Error Message**: Validation prevents item loading without equipment selection
- **Validation**: Only active equipment items are available
...
#### 5. **Quantity Input (Required for Request)**
...
#### 6. **Need Period Selection (Required for Request)**
...
#### **Equipment and Store Errors**
- **Error**: No equipment selected
- **Prevention**: Always select equipment before loading items
...
3. **Equipment must be configured** in the system
4. **Stores must have available items** for request
5. **Item types must be configured** with proper dispensing rules
6. **Request workflow must be enabled** for spare parts items

#### **Required System State**
- User authentication must be active
- Inventory request permissions must be configured
- Equipment data must be current
...
- ✅ Equipment dropdown populated with active equipment only
- ✅ Equipment validation ensures proper item filtering
- ✅ Equipment selection enables item loading
...
**Form Layout**: Right-to-left (RTL) Bootstrap form layout with vertical structure for spare parts request
...
<dx:BootstrapLayoutGroup ShowCaption="False" ColSpanMd="12">
    <Items>
        <dx:BootstrapLayoutItem Caption="رقم الطلب" ColSpanMd="4">
            <ContentCollection>
                <dx:ContentControl>
        ...
        <dx:BootstrapLayoutItem Caption="نوع الطلب" ColSpanMd="4">
            <ContentCollection>
                <dx:ContentControl>
                    <dx:BootstrapTextBox runat="server" ID="OrderType" Text=" 5 -طلب صرف قطع غيار  " ReadOnly="True"></dx:BootstrapTextBox>
                </dx:ContentControl>
            </ContentCollection>
        </dx:BootstrapLayoutItem>
        <dx:BootstrapLayoutItem Caption="المعدة" ColSpanMd="4" BeginRow="true">
            <ContentCollection>
                <dx:ContentControl>
                    <dx:BootstrapComboBox ID="Equipment" runat="server" TextFormatString="{1} - {0}" AutoPostBack="True" EnableMultiColumn="true" EnableCallbackMode="false" DataSourceID="EquipmentDS" ValueField="item_code" TextField="arabic_name">
                        <Fields>
        ...
        <dx:BootstrapLayoutItem Caption="الموظف" ColSpanMd="4">
            <ContentCollection>
                <dx:ContentControl>
...
<dx:BootstrapLayoutGroup ColSpanMd="12" ShowCaption="False">
    <Items>
        ...
        <dx:BootstrapLayoutItem Caption="نوع الصنف" ColSpanMd="12">
            <ContentCollection>
                <dx:ContentControl>
                    <dx:BootstrapComboBox ID="DespenseType" DataSourceID="DespenseTypeDS" Enabled="false" TextField="english_name" ValueField="id" OnSelectedIndexChanged="DespenseType_SelectedIndexChanged" runat="server" AutoPostBack="True" CallbackPageSize="15" EnableCallbackMode="false"></dx:BootstrapComboBox>
                </dx:ContentControl>
            </ContentCollection>
        </dx:BootstrapLayoutItem>
        ...
        <dx:BootstrapLayoutItem ColSpanMd="2" Caption="الكمية">
            <ContentCollection>
                <dx:ContentControl>
                    <dx:BootstrapSpinEdit runat="server" ID="Quentity" MinValue="0" AllowMouseWheel="false"></dx:BootstrapSpinEdit>
                </dx:ContentControl>
            </ContentCollection>
        </dx:BootstrapLayoutItem>
        ...
        <dx:ContentControl>
            <dx:BootstrapGridView ID="tempItems" AutoPostBack="true" ClientInstanceName="tempItems" KeyFieldName="id" DataSourceID="TempItemsDS" OnSelectionChanged="tempItems_SelectionChanged" ShowHeaderWhenEmpty="True" runat="server" Width="100%" SettingsBehavior-AllowSelectSingleRowOnly="true" AutoGenerateColumns="False" Styles-Cell-HorizontalAlign="Center" Styles-Cell-VerticalAlign="Middle" Styles-Header-VerticalAlign="Middle" Styles-Header-HorizontalAlign="Center" Font-Size="Small" EnableCallBacks="false" SettingsBehavior-ProcessFocusedRowChangedOnServer="true" SettingsBehavior-ProcessSelectionChangedOnServer="true">
                ...
                <Columns>
                    ...
                    <dx:BootstrapGridViewTextColumn FieldName="Item_Type_id" Visible="false" Caption="الكمية" VisibleIndex="1"></dx:BootstrapGridViewTextColumn>
                ...
                <TotalSummary>
                    <dx:ASPxSummaryItem FieldName="arabic_name" SummaryType="Count" DisplayFormat="كمية الاصناف =  {0 }" />
                    <dx:ASPxSummaryItem FieldName="Quntity" SummaryType="Sum" DisplayFormat="عدد الاصناف =  {0 }" />
                </TotalSummary>
        ...
        <dx:ContentControl>
            <dx:BootstrapGridView ID="ItemsHisroy" runat="server" AutoGenerateColumns="false" AutoPostBack="true" EnableCallBacks="false" SettingsBehavior-AllowSelectSingleRowOnly="true" OnSelectionChanged="ItemsHisroy_SelectionChanged" KeyFieldName="id" DataSourceID="ItemHis">
                ...
                <Columns>
                    ...
                    <dx:BootstrapGridViewDataColumn FieldName="EquipID" Caption="كود المعدة" VisibleIndex="1"></dx:BootstrapGridViewDataColumn>
...
<dx:BootstrapLayoutItem ShowCaption="False" ColSpanMd="12">
    <ContentCollection>
        <dx:ContentControl>
            <dx:BootstrapPopupControl runat="server" ID="PatientType" SettingsBootstrap-Sizing="Large" Width="1200" ShowCloseButton="true" Modal="true" HeaderText="تنبية" ClientInstanceName="popup" ShowHeader="true" ShowFooter="false" PopupHorizontalAlign="WindowCenter" PopupVerticalAlign="WindowCenter" CloseAction="CloseButton">
...
2. **Equipment Selection**: Loads equipment information
...
6. **Request Creation**: Adds items to temporary request grid
7. **Request Save**: Creates complete request records
8. **Request History**: Loads previous requests for user
...
- **Key Fields**: item_code, arabic_name, active, general_indecator
...
- **Key Fields**: id, OrderNo, OrderType, Date, Time, Emp, Dep, store_id, EquipID, Active, Approval, closed, Status
...
EquipmentDS.SelectCommand = "SELECT arabic_name, item_code FROM Inventories_Item_Settings WHERE (active = 1) and general_indecator=5";
...
ItemHis.SelectCommand = "SELECT id, OrderNo, OrderType, Date, Time, (select User_Name from Users where Emp_Code=convert(nvarchar,Emp)) Emp2,Emp, Dep,EquipID,(select arabic_name from Inventories_wharehouse_store where id=store_id) as arabic_name FROM Inventories_Dispense_Request_Header where Emp=@emp and Active = 1 and OrderType='5' and Approval=0 and closed=0 and Status='a'";
```

## Business Logic and Validation

### Equipment and Store Validation
...
**Equipment Logic**: Validates equipment selection before loading items
**Store Logic**: Validates store selection before loading items
**Error Prevention**: Prevents item loading without proper equipment and store context
...
- **Equipment Selection Validation**: Must select equipment before loading items
...
- **Equipment Validation**: Ensures equipment is active and available
...
- **Integration Details**:
  - Request workflow controlled by equipment and store selection
...
#### **Equipment and Store Information**
- **Database Tables**:
  - `Inventories_Item_Settings` - Item master data
  - `Inventories_wharehouse_store` - Store master data
- **Real-time Data**:
  - Equipment information for request
  - Store information for item filtering
  - Item quantities and availability
- **Data Relationships**:
  - Equipment linked to items via item type associations
...
- **Equipment Management**: Knowledge of equipment selection
...
2. **Equipment Selection**: Select equipment for request
3. **Store Selection**: Select store for request
...
8. **Item Addition**: Click add button to add item to temporary request grid
...
1. **Equipment Selection**: Select equipment for request
...
8. **Item Review**: Review items in temporary request grid
9. **Item Edit**: Edit items in temporary request grid
...
1. **Equipment Selection**: Select equipment for request
2. **Store Selection**: Select store for request
3. **Item Review**: Review all available items for selected store
