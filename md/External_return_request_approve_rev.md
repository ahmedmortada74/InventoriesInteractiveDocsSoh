...
# External_return_request_approve_rev.aspx

## Overview

**File**: `\Inventories\Process\External_return_request_approve_rev.aspx`
**Purpose**: Revision approval system for external return requests with approval and rejection capabilities
**Technology**: ASP.NET Web Forms with DevExpress Bootstrap controls (v18.2)
**Primary Users**: Revision approvers, inventory supervisors, revision approval personnel
...
- **Request Dropdown**: Must select valid return request for revision approval
...
- **Validation**: Only requests with return_done='1', return_rev_indator='0' are available
...
- **Prevention**: Verify request number is correct and has pending revision approval
- **Error**: Request already processed
- **Prevention**: Verify request has return_rev_indator='0' for approval

#### **Request Detail Errors**
- **Error**: No request details found
- **Prevention**: Ensure request has items pending revision approval
- **Error**: Request already approved
- **Prevention**: Verify request has return_rev_indator='0' for approval workflow
...
2. **User must have revision return approval permissions** via employee group assignments
3. **Return requests must be pending revision approval** with proper status
...
- Revision return approval permissions must be configured
- Return request workflow must be enabled
- Approval permissions must be configured
- Return requests must be pending revision approval

### Success Criteria

#### **For Request Selection**
- ✅ Request dropdown populated with pending revision approval requests only
...
#### **For Request Details**
- ✅ Request details grid displays all items pending revision approval
...
**Form Layout**: Right-to-left (RTL) Bootstrap form layout with vertical structure for revision return approval
...
3. **Request Display**: Shows all items pending revision approval
...
**Purpose**: Approves return request at revision level

**Process**:
1. Validates request selection
2. Updates request status to approved (return_rev_indator='1')
...
**Purpose**: Rejects return request at revision level

**Process**:
1. Validates request selection
2. Updates request status to rejected (return_rev_indator='2')
...
- **Purpose**: Return request records with revision approval tracking
- **Key Fields**: ID, PO_ID_FK, supplier_ID_FK, Itemcode, Price_unit, Total_Price, return_doc_id, Store_id, inv_num, Purchase_Id_unit, delivery_id_unit, Amount, Done_Amount, quantity_Amount_return, Expiration_date, doc_id, allow_Amount_return, return_done, return_rev_indator
- **Status Values**: return_done='1' (first approved), return_rev_indator='0' (pending revision), '1' (approved), '2' (rejected)
- **Usage**: Main table for revision return approval workflow
...
#### **Request Filtering**
```sql
select distinct return_doc_id from Inventories_return_to_suppliers where return_done='1' and return_rev_indator='0'
```

**Filtering Logic**: Shows only pending revision approval requests
**Permission Logic**: Only requests with return_rev_indator='0' are available
**Validation**: Ensures request is pending revision approval
...
SqlDataSource1.SelectCommand = "select distinct return_doc_id from Inventories_return_to_suppliers where return_done='1' and return_rev_indator='0'";
...
SqlDataSource2.SelectCommand = "select distinct Inventories_return_to_suppliers.ID,Inventories_return_to_suppliers.PO_ID_FK,purches_Supplier_record.Arabic_name,Inventories_Item_Settings.arabic_name as item,Inventories_return_to_suppliers.Price_unit,Inventories_return_to_suppliers.Total_Price,Inventories_return_to_suppliers.return_doc_id,Inventories_return_to_suppliers.Store_id,Inventories_return_to_suppliers.inv_num,purchases_unit.description as purchases_desc,Inventories_return_to_suppliers.Purchase_Id_unit,delivery_unit.description,Inventories_return_to_suppliers.delivery_id_unit,Inventories_return_to_suppliers.Amount,Inventories_return_to_suppliers.Done_Amount,quantity_Amount_return,Inventories_return_to_suppliers.Expiration_date,doc_id,allow_Amount_return from Inventories_return_to_suppliers inner join purches_Supplier_record on Supplier_code=supplier_ID_FK inner join Inventories_Item_Settings on Inventories_Item_Settings.item_code=Inventories_return_to_suppliers.Itemcode inner join Inventories_UOM purchases_unit on purchases_unit.id=Inventories_return_to_suppliers.Purchase_Id_unit inner join Inventories_UOM delivery_unit on delivery_unit.id=Inventories_return_to_suppliers.delivery_id_unit where Inventories_return_to_suppliers.return_doc_id=@PO_doc and return_done='1' and return_rev_indator='0'";
```
...
protected void Adding_Click1(object sender, EventArgs e)
{
    ...
    {
        cn.ExcuteSQL("update Inventories_return_to_suppliers set return_rev_indator='1' where return_doc_id ='" + rquest_return.Value.ToString() + "' ");
        BootstrapGridView1.DataBind();
...
**Approval Logic**: Updates request status to approved at revision level
**Selection Logic**: Validates request selection before approval
**Data Update**: Updates return_rev_indator field to '1' for approval
...
protected void reject_Click(object sender, EventArgs e)
{
    ...
    {
        cn.ExcuteSQL("update Inventories_return_to_suppliers set return_rev_indator='2' where return_doc_id ='" + rquest_return.Value.ToString() + "' ");
        BootstrapGridView1.DataBind();
...
**Rejection Logic**: Updates request status to rejected at revision level
**Selection Logic**: Validates request selection before rejection
**Data Update**: Updates return_rev_indator field to '2' for rejection
...
- **Request Status Validation**: Ensures request has pending revision approval status
- **Request Detail Validation**: Ensures items are pending revision approval
...
- **Database Tables**:
  - `Inventories_return_to_suppliers` - Return request records with revision approval tracking
- **Integration Details**:
...
- **Cause**: Request has no items pending revision approval
- **Solution**: Verify request has items with return_rev_indator='0'
- **Prevention**: Ensure request has proper approval workflow status

#### **Request Already Processed**
- **Cause**: Request has already been approved or rejected
- **Solution**: Verify request has return_rev_indator='0' for approval
- **Prevention**: Ensure request is pending revision approval
...
2. **Request Selection**: Select return request for revision approval
