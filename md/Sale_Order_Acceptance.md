...
# Sale_Order_Acceptance.aspx

## Overview

**File**: `\Inventories\Process\Sale_Order_Acceptance.aspx`
**Purpose**: Sale order acceptance page for initial approval of sale requests
...
- **Data Source**: Inventories_Sell_HD table with status='first' (pending acceptance)
...
- **Validation**: Only orders with status='first' (pending acceptance) are available
...
- **Error**: Sale order has no items pending acceptance
- **Prevention**: Verify sale order has items pending acceptance
...
2. **User must have acceptance permissions** via employee group assignments
3. **Sale orders must be pending acceptance** in the system
4. **Items must be available** for acceptance

#### **Required System State**
- User authentication must be active
- Acceptance permissions must be configured
...
- ✅ Sale order dropdown populated with pending acceptance orders only
...
**Form Layout**: Right-to-left (RTL) Bootstrap form layout with horizontal structure for sale order acceptance
...
3. Sets default acceptance state
...
- **Usage**: Tracks sale order information for acceptance
- **Filtering**: Only orders with status='first' (pending acceptance)
...
- **Usage**: Tracks sale order items for acceptance
...
**Validation**: Ensures user is authenticated before accessing acceptance data

#### **Order Filtering**
```sql
SELECT id FROM Inventories_Sell_HD WHERE status='first'
```

**Filtering Logic**: Shows only orders pending acceptance
**Permission Logic**: Only orders pending acceptance are available
**Validation**: Ensures order has items pending acceptance
...
orders.SelectCommand = "SELECT id FROM Inventories_Sell_HD WHERE status='first'";
...
- **Order Validation**: Ensures order is pending acceptance
- **Item Validation**: Ensures items are pending acceptance
...
- **Database Tables**:
  ...
  - `Inventories_Sell_DTL` - Sale order item details
- **Real-time Data**:
  - Order information for acceptance
  - Item quantities and prices
- **Data Relationships**:
  - Orders linked to items via Header_FK
  - Approval/rejection tracked by user

#### **Item and Acceptance Information**
- **Database Tables**:
  - `Inventories_Sell_DTL` - Sale order item details
  - `Inventories_Reasons` - Rejection reason master data
- **Real-time Data**:
  - Item details and descriptions
  - Acceptance quantities and prices
  - Unit information and calculations
- **Data Relationships**:
  - Items linked to acceptance via item_Code
  - Acceptance tracked by order
...
- **Prevention**: Order selection is required for all acceptance operations
...
#### **No Orders Found**
- **Cause**: No orders pending acceptance
- **Solution**: Verify orders are pending acceptance
- **Prevention**: Ensure orders are pending acceptance
...
- **Order Access**: Access to orders with acceptance items
...
- **Order Management**: Knowledge of order selection and acceptance
- **Acceptance Management**: Familiarity with acceptance save and rejection operations
...
2. **Order Selection**: Select order for acceptance
...
### Multi-Order Acceptance Management

1. **Order Selection**: Select order for acceptance
...
5. **Completion**: Complete all acceptance operations