...
# General_Dispense_Sub_saving.aspx

## Overview

**File**: `\Inventories\Process\General_Dispense_Sub_saving.aspx`
**Purpose**: Pharmacy dispensing system for saving/storing medications with prescription-based workflow
...
#### 5. **Batch Selection (Required for Dispensing)**
...
<dx:BootstrapLayoutGroup Caption="Request Header" ColSpanMd="12">
    <Items>
...
<dx:BootstrapLayoutGroup ShowCaption="False" SettingsItemCaptions-HorizontalAlign="Right" HorizontalAlign="Right" ColSpanMd="12">
    ...
    <Items>
        <dx:BootstrapLayoutItem Caption="الاسم التجارى" CaptionSettings-HorizontalAlign="Right" ColSpanMd="2">
            <ContentCollection>
                <dx:ContentControl>
                    <dx:BootstrapComboBox ID="inv_no" runat="server" AutoPostBack="True" CallbackPageSize="15" EnableCallbackMode="True" DataSourceID="ItemDS" ValueField="item_code" TextField="arabic_name" OnSelectedIndexChanged="inv_no_SelectedIndexChanged"></dx:BootstrapComboBox>
                </dx:ContentControl>
            </ContentCollection>
        </dx:BootstrapLayoutItem>
...
- **Key Fields**: ID, store, patch, Quntitiy, item, emp, date, ActiveCode, Category, Frequency, Duration_no, Duration_unit, Administration_Instructions, Dose_After, dose_unit, Dose, IvCode, IVName, ProtocolID, Count_Lable, PharmDose_FK, Expiration_date, FK_order_no
...
ItemDS.SelectCommand = "SELECT arabic_name, item_code FROM Inventories_Item_Settings inner join Inventories_Item_Settings_drug_sheet drug_sheet on Inventories_Item_Settings.id = drug_sheet.Item_Settings_fk WHERE Inventories_Item_Settings.active = 1 and drug_sheet.cb_effective_materails=@effective";
...
8. **Batch Selection**: Select batch from popup for item
...
12. **Dispensing Save**: Click save button to create complete dispensing request
...
6. **Item Selection**: Select item from available medications dropdown
...
10. **Item Review**: Review items in temporary dispensing grid
11. **Item Deletion**: Remove items from temporary dispensing grid
12. **Dispensing Completion**: Save dispensing with all validated items
...
9. **Batch Management**: Manage batches for each item
10. **Dispensing Validation**: Ensure all items have proper validation
11. **Dispensing Save**: Save dispensing with all validated items