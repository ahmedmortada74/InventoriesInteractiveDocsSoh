...
# Sale_Order_Receving.aspx

## Overview

**File**: `\Inventories\Process\Sale_Order_Receving.aspx`
**Purpose**: Sale order receiving page for confirming inspection and receiving of sale requests
**Technology**: ASP.NET Web Forms with DevExpress Bootstrap controls (v18.2)
**Primary Users**: Warehouse staff, inventory administrators, receiving personnel
...
- **Data Source**: Inventories_Sell_HD table with status='fifth' (pending receiving)
...
- **Validation**: Only orders with status='fifth' (pending receiving) are available
...
- **Confirm Button**: Must click confirm button to confirm inspection and receiving
...
- **Error**: Sale order has no items pending receiving
- **Prevention**: Verify sale order has items pending receiving
...
2. **User must have receiving permissions** via employee group assignments
3. **Sale orders must be pending receiving** in the system
4. **Items must be available** for receiving

#### **Required System State**
- User authentication must be active
- Receiving permissions must be configured
...
- ✅ Sale order dropdown populated with pending receiving orders only
...
- ✅ Order selection enables confirmation workflow
...
**Form Layout**: Right-to-left (RTL) Bootstrap form layout with horizontal structure for sale order receiving
        ...
        <dx:ContentControl>
            <dx:BootstrapGridView ID="grid" runat="server" EnableCallBacks="false" AutoPostBack="true" KeyFieldName="id" CssClasses-Control="cc" OnCustomColumnDisplayText="grid_CustomColumnDisplayText" DataSourceID="details">
                <Columns>
                    ...
                    <dx:BootstrapGridViewDataColumn FieldName="inventory_code" Visible="false" VisibleIndex="5" />
                    ...
                    <dx:BootstrapGridViewDataColumn FieldName="StockId" Visible="false" VisibleIndex="15" />
                </Columns>
...
<dx:BootstrapLayoutItem Caption="" ColSpanMd="4">
    <ContentCollection>
        <dx:ContentControl>
            <dx:BootstrapButton ID="btn_approve" runat="server" AutoPostBack="true" EnableCallbackMode="false" Text="تأكيد الفحص و الاستلام" Width="100%" OnClick="btn_approve_Click">
...
4. **Confirmation**: Processes confirmation for selected sale order
...
3. Sets default receiving state
...
**Purpose**: Confirms inspection and receiving for selected sale order
...
- **Usage**: Tracks sale order information for receiving
- **Filtering**: Only orders with status='fifth' (pending receiving)
...
- **Usage**: Tracks sale order items for receiving
...
**Validation**: Ensures user is authenticated before accessing receiving data

#### **Order Filtering**
```sql
SELECT id FROM Inventories_Sell_HD WHERE status='fifth'
```

**Filtering Logic**: Shows only orders pending receiving
**Permission Logic**: Only orders pending receiving are available
**Validation**: Ensures order has items pending receiving
...
orders.SelectCommand = "SELECT id FROM Inventories_Sell_HD WHERE status='fifth'";
...
details.SelectCommand = "SELECT dt.id, item_Name, StockId, inventory_code, item_Code, batch_No, inventory_code, inventory_Name, exp_Date, storage_Unit, available_amount, price_Unit, total_PriceAvailableAmount, SupplierCode, SupplyInvoiceNumber, sellQuantity, BuyerCode, BuyerName, StockId, dt.TotalSellValue FROM Inventories_Sell_DTL dt INNER JOIN Inventories_Sell_HD hd ON hd.id=dt.Header_FK WHERE Header_FK=@Header_FK AND hd.status='fifth'";
...
- **Order Validation**: Ensures order is pending receiving
- **Item Validation**: Ensures items are pending receiving
...
- **Receiving Access**: Ensures user can access and modify receiving records
...
- **Confirmation Success**: "تم تأكيد الفحص و الاستلام" (Inspection and receiving confirmed successfully)
...
- **Permission System**:
  - User authentication required for all receiving operations
...
- **Database Tables**:
  ...
  - `Inventories_Sell_DTL` - Sale order item details
- **Real-time Data**:
  - Order information for receiving
  - Item quantities and prices
- **Data Relationships**:
  - Orders linked to items via Header_FK
  - Confirmation/rejection tracked by user

#### **Item and Receiving Information**
- **Database Tables**:
  - `Inventories_Sell_DTL` - Sale order item details
  - `Inventories_Reasons` - Rejection reason master data
- **Real-time Data**:
  - Item details and descriptions
  - Receiving quantities and prices
  - Unit information and calculations
- **Data Relationships**:
  - Items linked to receiving via item_Code
  - Receiving tracked by order
...
- **Prevention**: Order selection is required for all receiving operations
...
#### **No Orders Found**
- **Cause**: No orders pending receiving
- **Solution**: Verify orders are pending receiving
- **Prevention**: Ensure orders are pending receiving
...
- **Receiving Access**: Access to receiving operations
- **Order Access**: Access to orders with receiving items
...
- **Receiving Workflow**: Understanding of receiving process
- **Order Management**: Knowledge of order selection and receiving
- **Receiving Management**: Familiarity with receiving save and rejection operations
...
2. **Order Selection**: Select order for receiving
...
### Multi-Order Receiving Management

1. **Order Selection**: Select order for receiving
2. **Detail Review**: Review items for selected order
3. **Confirmation**: Process confirmation for selected order
4. **Rejection**: Process rejection for selected order with reason
5. **Completion**: Complete all receiving operations