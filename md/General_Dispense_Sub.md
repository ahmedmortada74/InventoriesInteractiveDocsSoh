...
# General_Dispense_Sub.aspx

## Overview

**File**: `\Inventories\Process\General_Dispense_Sub.aspx`
**Purpose**: Pharmacy dispensing system for patient medications with prescription-based workflow and batch selection
...
<dx:BootstrapLayoutGroup ShowCaption="False" ColSpanMd="12">
    <Items>
        ...
        <dx:BootstrapLayoutItem Caption="المخزن" ColSpanMd="4">
            <ContentCollection>
                <dx:ContentControl>
                    <dx:BootstrapComboBox runat="server" ID="typeHeader" DataSourceID="typeHeaderDS" AutoPostBack="false" TextField="arabic_name" ValueField="code" OnSelectedIndexChanged="typeHeader_SelectedIndexChanged"></dx:BootstrapComboBox>
                </dx:ContentControl>
            </ContentCollection>
        </dx:BootstrapLayoutItem>
...
- **Key Fields**: ID, store, patch, Quntitiy, item, emp, date, ActiveCode, Category, Frequency, Duration_no, Duration_unit, Administration_Instructions, Dose_After, dose_unit, Dose, IvCode, IVName, ProtocolID, Count_Lable, PharmDose_FK, Expiration_date, FK_order_no, ProtocolType
...
ItemDS.SelectCommand = "SELECT Inventories_Item_Settings.item_code, CONCAT(Inventories_Item_Settings.arabic_name, ' - ', concentration, Inventories_UOM.mnemonic, ' - ', sum(Quantity_Exchange)-SUM(Amount_Done_Exchange)) as arabic_name,Expiration_date,(SUM(Quantity_Exchange - Amount_Done_Exchange) ) remain FROM Inventories_Item_Settings inner join Inventories_Item_Settings_drug_sheet drug_sheet on Inventories_Item_Settings.id = drug_sheet.Item_Settings_fk left join Inventories_UOM on Inventories_UOM.id = drug_sheet.concentration_unit left join Inventories_Stock on Inventories_Stock.Itemcode = Inventories_Item_Settings.item_code WHERE Inventories_Stock.storeid=@STORE and MoveType in (select procedure_id from Inventories_procedures_orderEffect where quantity_effect=1) and Inventories_Item_Settings.active = 1 and drug_sheet.cb_effective_materails=@effective and Expiration_date not in (select Expiration_date FROM Inventories_General_Dispense_temp where emp=@emp and item=Inventories_Item_Settings.item_code and Quntitiy = (select (SUM(Quantity_Exchange) - SUM(Amount_Done_Exchange) ) from Inventories_Stock where Inventories_Stock.Expiration_date = Inventories_General_Dispense_temp.Expiration_date and Itemcode= Inventories_Item_Settings.item_code and storeid=@STORE ) and date=@date) group by item_code,arabic_name,concentration,mnemonic,Expiration_date having SUM(Quantity_Exchange - Amount_Done_Exchange) >0";
...
12. **Item Deletion**: Remove items from temporary dispensing grid
13. **Dispensing Completion**: Save dispensing with all validated items
...
4. **Store Selection**: Select store for dispensing
5. **Search Execution**: Click search button to load prescriptions
6. **Prescription Review**: Review all prescriptions in prescription items grid
7. **Selective Dispensing**: Add specific items from different prescriptions as needed
8. **Quantity Management**: Manage dispensing quantities for each item
9. **Store Management**: Manage stores for each item
10. **Batch Management**: Manage batches for each item
11. **Dispensing Validation**: Ensure all items have proper validation
12. **Dispensing Save**: Save dispensing with all validated items
