...
# AdministrationDismissalRequestFixedAsset.aspx

## Overview

**File**: `\Inventories\Process\AdministrationDismissalRequestFixedAsset.aspx`
**Purpose**: Fixed asset dismissal request system for inventory items with item selection and quantity management
**Technology**: ASP.NET Web Forms with DevExpress Bootstrap controls (v18.2)
**Primary Users**: Inventory administrators, fixed asset managers, department supervisors
...
#### 6. **Operation User Selection (Required for Request)**
- **Operation User Dropdown**: Must select valid operation user for request
- **Error Prevention**: System validates operation user is selected before saving request
- **Data Source**: Users table with employee codes
- **Default Behavior**: User must select operation user manually
- **Error Message**: Validation prevents request creation without operation user selection
- **Validation**: Only active users are available
...
#### **Need Period and Operation User Errors**
...
- **Error**: No operation user selected
- **Prevention**: Always select operation user before saving request
...
5. **Request workflow must be enabled** for fixed asset dismissal items
...
**Form Layout**: Right-to-left (RTL) Bootstrap form layout with vertical structure for fixed asset dismissal request
...
<dx:BootstrapLayoutGroup ShowCaption="False" ColSpanMd="12">
    <Items>
        ...
        <dx:BootstrapLayoutItem Caption="نوع الطلب" ColSpanMd="3">
            <ContentCollection>
                <dx:ContentControl>
                    <dx:BootstrapTextBox runat="server" ID="OrderType" Text=" 6 - طلب صرف اصل ثابت  " ReadOnly="True"></dx:BootstrapTextBox>
                </dx:ContentControl>
            </ContentCollection>
        </dx:BootstrapLayoutItem>
        <dx:BootstrapLayoutItem Caption="الموظف" ColSpanMd="3" BeginRow="true">
            <ContentCollection>
                <dx:ContentControl>
        ...
        <dx:BootstrapLayoutItem Caption="مسئول التشغيل" ColSpanMd="3">
            <ContentCollection>
                <dx:ContentControl>
                    <dx:BootstrapComboBox ID="OperationUser" runat="server" TextFormatString="{0} - {1}" AutoPostBack="True" EnableMultiColumn="true" CallbackPageSize="15" EnableCallbackMode="false" DataSourceID="OperationUserDS" ValueField="Emp_Code" TextField="User_Name">
                        <Fields>
                            <dx:BootstrapListBoxField FieldName="Emp_Code" />
                            <dx:BootstrapListBoxField FieldName="User_Name" />
                        </Fields>
        ...
        <dx:ContentControl>
            <dx:BootstrapGridView ID="ItemsHisroy" runat="server" AutoGenerateColumns="false" AutoPostBack="true" EnableCallBacks="false" SettingsBehavior-AllowSelectSingleRowOnly="true" OnSelectionChanged="ItemsHisroy_SelectionChanged" KeyFieldName="id" DataSourceID="ItemHis">
                ...
                <Columns>
                    ...
                    <dx:BootstrapGridViewDataColumn FieldName="OpUser" Caption="مسئول التشغيل" Visible="false"></dx:BootstrapGridViewDataColumn>
                    <dx:BootstrapGridViewDataColumn FieldName="OpUser2" Caption="مسئول التشغيل" VisibleIndex="1"></dx:BootstrapGridViewDataColumn>
                    <dx:BootstrapGridViewDataColumn FieldName="RespnsableUser" Caption="الموظف المسئول" Visible="false"></dx:BootstrapGridViewDataColumn>
                    <dx:BootstrapGridViewDataColumn FieldName="RespnsableUser2" Caption="الموظف المسئول" VisibleIndex="1" Visible="false"></dx:BootstrapGridViewDataColumn>
...
- **Key Fields**: id, OrderNo, OrderType, Date, Time, Emp, Dep, store_id, OpUser, RespnsableUser, Active, Approval, closed, Status
...
- **Filtering**: Excludes system users ('0', '00', '009')
...
ItemHis.SelectCommand = "SELECT id, OrderNo, OrderType, Date, Time,Dep, (select User_Name from Users where Emp_Code=convert(nvarchar,OpUser)) OpUser2, OpUser,(select User_Name from Users where Emp_Code=convert(nvarchar,RespnsableUser)) RespnsableUser2,RespnsableUser,(select User_Name from Users where Emp_Code=convert(nvarchar,Emp)) Emp2,Emp,(select arabic_name from Inventories_wharehouse_store where id=store_id) as arabic_name FROM Inventories_Dispense_Request_Header where Emp=@emp and Active = 1 and OrderType=6 and Approval=0 and closed=0 and Status='a'";
```
