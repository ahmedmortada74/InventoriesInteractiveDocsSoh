...
# General_Dispense_saving.aspx

## Overview

**File**: `\Inventories\Process\General_Dispense_saving.aspx`
**Purpose**: General dispensing system for saving/storing inventory items with request-based workflow and batch selection
...
- **Data Source**: Inventories_Dispense_Request_Header table with active requests
...
- **Validation**: Only active requests (Status='a') are available

#### 4. **Item Selection (Required for Dispensing)**
...
- **Data Source**: Inventories_Dispense_Request_Details table with request items
...
- **Validation**: Only items with active rules are available

#### 5. **Store Selection (Required for Batch)**
- **Store Dropdown**: Must select valid store for batch filtering
- **Error Prevention**: System validates store is selected before loading batches
- **Data Source**: Inventories_wharehouse_store table linked with store rules
- **Default Behavior**: User must select store manually
- **Error Message**: Validation prevents batch loading without store selection
...
- **Prevention**: Verify request has Status='a'

#### **Store and Batch Errors**
- **Error**: No store selected
- **Prevention**: Always select store before loading batches
...
6. **Requests must be active** with proper status
...
- ✅ Request dropdown populated with active requests only
- ✅ Request details display properly in grid
- ✅ Request validation prevents item loading without selection
- ✅ Request status shows current dispensing state

#### **For Item Selection**
- ✅ Item grid displays all items with request quantities
...
- ✅ Print functionality works with report viewer
...
<dx:BootstrapLayoutGroup ShowCaption="False" SettingsItemCaptions-HorizontalAlign="Right" HorizontalAlign="Right" ColSpanMd="12">
    ...
    <Items>
        <dx:BootstrapLayoutItem Caption="المخزن" CaptionSettings-HorizontalAlign="Right" ColSpanMd="2">
            <ContentCollection>
                <dx:ContentControl>
                    <dx:BootstrapComboBox ID="inv_no" runat="server" AutoPostBack="True" CallbackPageSize="15" EnableCallbackMode="True" DataSourceID="StoresPerDS" ValueField="code" TextField="english_name" OnSelectedIndexChanged="inv_no_SelectedIndexChanged"></dx:BootstrapComboBox>
                </dx:ContentControl>
            </ContentCollection>
        </dx:BootstrapLayoutItem>
...
4. **Request Selection**: Loads request items with quantities
5. **Item Selection**: Loads store information for selected item
6. **Store Selection**: Loads batch information for selected item
...
**Purpose**: Loads store information for selected item

**Process**:
1. Validates item selection
2. Retrieves item details from grid
3. Sets parameters for store data source
4. Binds store dropdown
...
**Purpose**: Loads batch information for selected store
...
**Process**:
1. Validates item selection
2. Validates store selection
3. Shows batch popup
4. Binds batch grid with available batches
...
- **Key Fields**: id, arabic_name, english_name, active, Store_type, Inventory_Type
...
- **Key Fields**: id, OrderNo, OrderType, Active, Status
- **Usage**: Tracks dispensing requests
- **Filtering**: Only active requests (Status='a')
...
- **Filtering**: Only items with active rules
...
#### **Inventories_Stock_Saving**
- **Purpose**: Stock saving records with batch tracking
- **Key Fields**: ID, Itemcode, storeid, Quantity_Exchange, Amount_Done_Exchange, Expiration_date
...
and WS.Inventory_Type =@inv and WS.id in (select ws.id from Inventories_Procedures_Stores_DTL sd  
...
where sd.Active =1 and sh.Setup_Procedure_FK = 3 and Inventories_rules_movement.emp_id =@emp and Inventories_rules_movement.active =1) and Store_type=@type
```
...
RequestDS.SelectCommand = "SELECT distinct H.id, OrderNo FROM Inventories_Dispense_Request_Header H inner join Inventories_Dispense_Request_Details on H.id = Header_FK where Active = 1 and OrderType=@type and [Status] ='a'";
...
RequstItemsDS.SelectCommand = "SELECT distinct Temp.Item_Type_id,Temp.id,Done, Remain,IIS.arabic_name,ISett.arabic_name Discription,IIS.item_code, IIS.item_code, Quntity, Due_Date,(SELECT description FROM Inventories_Item_Settings S inner join Inventories_UOM_item_unit U on U.item_code = S.item_code inner join Inventories_UOM UO on unit_id =UO.id where unit_type_id=3 and S.item_code = IIS.item_code) as unit FROM Inventories_Dispense_Request_Details Temp inner join Inventories_Item_Settings IIS on IIS.item_code = Temp.item_code inner join Inventories_item_type ISett inner Join Inventories_rules_items_type on Inventories_rules_items_type.Items_Type_id = ISett.id on ISett.id = Temp.Item_Type_id WHERE Temp.Header_FK=@header and Inventories_rules_items_type.active=1";
...
StoresPerDS.SelectCommand = "SELECT Inventories_rules_stores.id, WS.id as code,english_name, arabic_name, Store_type FROM Inventories_wharehouse_store WS inner join Inventories_rules_stores on store_id = WS.id WHERE Inventories_rules_stores.active = 1 and (WS.active = 1) and emp_id = @emp and WS.Inventory_Type =@inv and WS.id in (select ws.id from Inventories_Procedures_Stores_DTL sd inner join Inventories_Procedures_Stores_HD sh on sd.Setup_Procedure_Stores_FK =sh.ID inner join Inventories_wharehouse_store ws on ws.id = sd.Stores_Code inner join Inventories_rules_movement on sh.Setup_Procedure_FK = Inventories_rules_movement.Movement_id where sd.Active =1 and sh.Setup_Procedure_FK = 3 and Inventories_rules_movement.emp_id =@emp and Inventories_rules_movement.active =1) and Store_type=@type";
...
batchGridViewDS.SelectCommand = "select ID,Expiration_date,(Quantity_Exchange - SUM(Amount_Done_Exchange) ) remain from Inventories_Stock_Saving where Itemcode=@code and storeid=@store and MoveType in (select procedure_id from Inventories_procedures_orderEffect where quantity_effect=1) and ID not in (select patch FROM Inventories_General_Dispense_temp where emp=@emp and item=@code and Quntitiy = (select Quantity_Exchange - Amount_Done_Exchange from Inventories_Stock_Saving where ID = patch ) and date=@date) group by ID,Expiration_date,Quantity_Exchange having (Quantity_Exchange - SUM(Amount_Done_Exchange) ) > 0 order by Expiration_date";
...
**Error Prevention**: Prevents item loading without proper request context
...
**Store Logic**: Validates store selection before loading batches
**Error Prevention**: Prevents batch loading without proper store selection
...
- **Store Selection Validation**: Must select store before loading batches
...
- **Request Validation**: Ensures request is active and not closed
...
- **Database Tables**:
  ...
  - `Inventories_Dispense_Request_Details` - Request details with quantities
- **Integration Details**:
...
- **Cause**: Store not selected before loading batches
- **Solution**: Always select store before loading batches
...
4. **Request Selection**: Select active request for dispensing
...
3. **Request Selection**: Select active request for dispensing
...
1. **Store Type Selection**: Select store type for dispensing
2. **Request Type Selection**: Select request type for dispensing
3. **Request Selection**: Select active request with multiple items
4. **Item Review**: Review all items in request items grid
5. **Selective Dispensing**: Add specific items as needed
6. **Quantity Management**: Manage dispensing quantities for each item
7. **Store Management**: Manage stores for each item
