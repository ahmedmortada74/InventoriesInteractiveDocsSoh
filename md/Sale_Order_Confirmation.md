...
# Sale_Order_Confirmation.aspx

## Overview

**File**: `\Inventories\Process\Sale_Order_Confirmation.aspx`
**Purpose**: Sale order confirmation page for final confirmation of sale requests
...
- **Data Source**: Inventories_Sell_HD table with status='third' (pending confirmation)
...
- **Validation**: Only orders with status='third' (pending confirmation) are available
...
- **Confirm Button**: Must click confirm button to confirm sale order
...
- **Error**: Sale order has no items pending confirmation
- **Prevention**: Verify sale order has items pending confirmation
...
3. **Sale orders must be pending confirmation** in the system
...
- ✅ Sale order dropdown populated with pending confirmation orders only
...
**Form Layout**: Right-to-left (RTL) Bootstrap form layout with horizontal structure for sale order confirmation
...
<dx:BootstrapLayoutItem Caption="" ColSpanMd="4">
    <ContentCollection>
        <dx:ContentControl>
            <dx:BootstrapButton ID="btn_approve" runat="server" AutoPostBack="true" EnableCallbackMode="false" Text="تأكيد المراجعة" Width="100%" OnClick="btn_approve_Click">
...
**Purpose**: Confirms selected sale order
...
- **Usage**: Tracks sale order information for confirmation
- **Filtering**: Only orders with status='third' (pending confirmation)
...
- **Usage**: Tracks sale order items for confirmation
...
#### **Order Filtering**
```sql
SELECT id FROM Inventories_Sell_HD WHERE status='third'
```
...
orders.SelectCommand = "SELECT id FROM Inventories_Sell_HD WHERE status='third'";
...
- **Confirmation Success**: "تم تأكيد المراجعة" (Review confirmed successfully)
