...
# External_return_request_approve.aspx

## Overview

**File**: `\Inventories\Process\External_return_request_approve.aspx`
**Purpose**: External return request approval system for inventory items with approval and rejection capabilities
**Technology**: ASP.NET Web Forms with DevExpress Bootstrap controls (v18.2)
**Primary Users**: Return request approvers, inventory supervisors, approval personnel
...
- **Request Dropdown**: Must select valid return request for approval
- **Error Prevention**: System validates request is selected before loading details
- **Data Source**: Inventories_return_to_suppliers table filtered by pending status
...
- **Validation**: Only requests with return_done='0' (pending) are available
...
- **Prevention**: Verify request has return_done='0' for approval

#### **Request Detail Errors**
- **Error**: No request details found
- **Prevention**: Ensure request has items pending approval
- **Error**: Request already approved
- **Prevention**: Verify request has return_done='0' for approval workflow
...
3. **Return requests must be pending approval** with proper status
...
- Return requests must be pending approval

### Success Criteria

#### **For Request Selection**
- ✅ Request dropdown populated with pending return requests only
...
**Form Layout**: Right-to-left (RTL) Bootstrap form layout with vertical structure for return approval
...
3. **Request Display**: Shows all items pending approval
...
**Purpose**: Approves return request

**Process**:
1. Validates request selection
2. Updates request status to approved (return_done='1')
...
**Purpose**: Rejects return request

**Process**:
1. Validates request selection
2. Updates request status to rejected (return_done='2')
...
- **Purpose**: Return request records with approval tracking
- **Key Fields**: ID, PO_ID_FK, supplier_ID_FK, Itemcode, Price_unit, Total_Price, return_doc_id, Store_id, inv_num, Purchase_Id_unit, delivery_id_unit, Amount, Done_Amount, quantity_Amount_return, Expiration_date, doc_id, allow_Amount_return, return_done
- **Status Values**: return_done='0' (pending), '1' (approved), '2' (rejected)
- **Usage**: Main table for return approval workflow
...
#### **Request Filtering**
```sql
select distinct return_doc_id from Inventories_return_to_suppliers where return_done='0'
```

**Filtering Logic**: Shows only pending return requests
**Permission Logic**: Only requests with return_done='0' are available
**Validation**: Ensures request is pending approval
...
SqlDataSource1.SelectCommand = "select distinct return_doc_id from Inventories_return_to_suppliers where return_done='0'";
...
protected void Adding_Click1(object sender, EventArgs e)
{
    ...
    {
        cn.ExcuteSQL("update Inventories_return_to_suppliers set return_done='1' where return_doc_id ='" + rquest_return.Value.ToString() + "' ");
        BootstrapGridView1.DataBind();
...
**Data Update**: Updates return_done field to '1' for approval
...
protected void reject_Click(object sender, EventArgs e)
{
    ...
    {
        cn.ExcuteSQL("update Inventories_return_to_suppliers set return_done='2' where return_doc_id ='" + rquest_return.Value.ToString() + "' ");
        BootstrapGridView1.DataBind();
...
**Data Update**: Updates return_done field to '2' for rejection
...
- **Database Tables**:
  - `Inventories_return_to_suppliers` - Return request records with approval tracking
- **Integration Details**:
...
- **Solution**: Verify request has items with return_done='0'
- **Prevention**: Ensure request has proper approval workflow status

#### **Request Already Processed**
- **Cause**: Request has already been approved or rejected
- **Solution**: Verify request has return_done='0' for approval
- **Prevention**: Ensure request is pending approval
...
2. **Request Selection**: Select return request for approval
