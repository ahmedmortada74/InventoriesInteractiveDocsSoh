...
# Internal_return_request_Previous_years.aspx

## Overview

**File**: `\Inventories\Process\Internal_return_request_Previous_years.aspx`
**Purpose**: Internal return request system for previous years' inventory items with dispensing-based workflow
...
#### 4. **Dispense Document Selection (Required for Request)**
...
- **Data Source**: Inventory_internal_Return_Previous_years stored procedure
...
#### 5. **Item Selection (Required for Request)**
...
#### 6. **Return Quantity Input (Required for Request)**
...
#### **Financial Year and Dispense Document Errors**
...
6. **Dispense documents must be available** for request
...
<dx:BootstrapLayoutGroup Caption="طلب الارتجاع الداخلى" SettingsItemCaptions-HorizontalAlign="Right" HorizontalAlign="Right" ColSpanMd="12">
    <Items>
        ...
        <dx:BootstrapLayoutItem Caption="السنة المالية" ColSpanMd="4">
            <ContentCollection>
                <dx:ContentControl>
...
7. **Return Quantity Entry**: Validates return quantity
8. **Return Reason Selection**: Validates return reason
9. **Request Creation**: Adds items to temporary request grid
10. **Request Save**: Creates complete request records
11. **Request History**: Loads previous requests for user
...
#### **Inventories_Internal_Return_temp_Previous_years**
...
ExMove.SelectCommand = "[Inventory_internal_Return_Previous_years]";
...
SqlDataSource5.SelectCommand = "[Inventory_internal_gridview_Previous_years]";
...
SqlDataSource2.SelectCommand = "select distinct (SELECT Dep_Name FROM Orman.dbo.DefinitionDep WHERE DepID=Dep) Depname,(select User_Name from Users where Emp_Code=convert(nvarchar,RespnsableUser)) RespnsableUser2,(select User_Name from Users where Emp_Code=convert(nvarchar,Emp)) Emp2,Emp,(SELECT arabic_name FROM Inventories_Item_Settings where item_code=EquipID) EquipName,Account,fileid,Inventories_Internal_Return_temp_Previous_years.doc_id,Inventories_Internal_Return_temp_Previous_years.ID,Inventories_Internal_Return_temp_Previous_years.PO_ID_FK,Inventories_Internal_Return_temp_Previous_years.Itemcode,Inventories_Item_Settings.arabic_name,Inventories_Internal_Return_temp_Previous_years.Price_unit,Inventories_Internal_Return_temp_Previous_years.Grand_Total,Inventories_Internal_Return_temp_Previous_years.doc_id,Inventories_Internal_Return_temp_Previous_years.storeid,Inventories_Internal_Return_temp_Previous_years.inv_num,Exchange_unit.description as Exchange_unit,Store_unit.description as Store_unit,Inventories_Internal_Return_temp_Previous_years.Amount_Done_Exchange,Inventories_Internal_Return_temp_Previous_years.Amount_Done,Inventories_Internal_Return_temp_Previous_years.Expiration_date,batch_no from Inventories_Internal_Return_temp_Previous_years inner join Inventories_Item_Settings on Inventories_Item_Settings.item_code=Inventories_Internal_Return_temp_Previous_years.Itemcode inner join Inventories_UOM Exchange_unit on Exchange_unit.id=Inventories_Internal_Return_temp_Previous_years.ItemUnit_Exchange_id inner join Inventories_UOM Store_unit on Store_unit.id=Inventories_Internal_Return_temp_Previous_years.ItemUnit_storage_Id left join Inventories_Dispense_Request_Details details on details.Header_FK =Inventories_Internal_Return_temp_Previous_years.dispense_request left join Inventories_Dispense_Request_Header head on head.id = details.Header_FK where Inventories_Internal_Return_temp_Previous_years.doc_id=@ID and (MoveType = 3) and Exchange_Status='A' and storeid=@store and Request=1 and Approve=0 and Return_Accepance=0 and Request_Emp =@emp and StockDate=@date";
...
ItemHis.SelectCommand = "SELECT ID, Date, Time, Emp, Active, FileId, Account, Status, storeID, docID FROM Inventories_Internal_Return_Header_Previous_years WHERE storeID=@store and docID=@ID and Status = 1 and active = 1";
```
...
**Financial Year Logic**: Validates financial year selection before request
**Error Prevention**: Prevents request without proper financial year selection
...
- **Database Tables**:
  - `Inventories_Internal_Return_temp_Previous_years` - Temporary return records before save
...
- **Prevention**: Financial year selection is required for all request operations
...
5. **Dispense Document Selection**: Select dispense document for request
6. **Item Review**: Review items in return items grid
7. **Item Selection**: Select specific items for request
8. **Return Quantity Entry**: Enter return quantity within limits
9. **Return Reason Selection**: Select return reason
10. **Item Addition**: Click add button to add item to temporary request grid
11. **Repeat Items**: Add additional request items as needed
12. **Request Save**: Click save button to create complete request
...
9. **Item Addition**: Add item to temporary request grid
10. **Item Review**: Review items in temporary request grid
...
5. **Selective Request**: Request specific returns as needed
6. **Return Quantity Management**: Manage return quantities for each item
7. **Return Reason Management**: Manage return reasons for each item
8. **Request Validation**: Ensure all returns have proper validation
9. **Request Completion**: Complete all request operations