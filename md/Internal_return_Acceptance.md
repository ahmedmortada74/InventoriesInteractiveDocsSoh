...
# Internal_return_Acceptance.aspx

## Overview

**File**: `\Inventories\Process\Internal_return_Acceptance.aspx`
**Purpose**: Internal return acceptance system for inventory items with approval workflow
...
#### 3. **Dispense Document Selection (Required for Acceptance)**
...
#### 4. **Item Selection (Required for Acceptance)**
...
5. **Dispense documents must be pending** for acceptance
...
6. **Acceptance**: Processes acceptance for selected items
...
ExMove.SelectCommand = "select distinct stock.doc_id from Inventories_Stock as stock inner join Inventories_Internal_Return as Raturn on Raturn.Stock_ID = stock.ID and stock.storeid=Raturn.storeid where stock.MoveType='3' and stock.storeid=@store and stock.Exchange_Status='A' and Raturn.Approve=1 and Raturn.Return_Accepance=0";
...
7. **Acceptance**: Click accept button to process acceptance
...
3. **Dispense Document Selection**: Select dispense document for acceptance
4. **Item Review**: Review items in return items grid
5. **Item Selection**: Select specific items for acceptance
6. **Acceptance**: Process acceptance for selected items
7. **Rejection**: Process rejection for selected items with reason
8. **Completion**: Complete all acceptance operations
